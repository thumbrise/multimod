...
type Module struct {
	// Path is the module path (e.g. "github.com/example/project/otel").
	Path string
	...
	// Dev-state: populated (local replace for all internal modules).
	// Publish-state: empty (replaces stripped).
	Replaces []string

	// RequireVersions maps internal module path → desired version in require directives.
	// Dev-state: nil (Applier does not touch require versions).
	// Publish-state: pinned (e.g. "example.com/root" → "v1.2.3").
	RequireVersions map[string]string
}

// State is the desired state of a multi-module Go project.
// Built by Discovery (validated and enriched).
...
// InternalPaths returns the complete set of all module paths in this project.
// Guaranteed: root + all subs.
func (s State) InternalPaths() map[string]bool {
	paths := make(map[string]bool, 1+len(s.Subs))
	paths[s.Root.Path] = true
...
// ModuleDir returns the directory of a module by its path.
// Returns empty AbsDir if the module is not part of this project.
func (s State) ModuleDir(modPath string) AbsDir {
	if modPath == s.Root.Path {
		return s.Root.Dir
	}
...
var defaultExcludedPrefixes = []string{"."} //nolint:gochecknoglobals // package-level defaults
